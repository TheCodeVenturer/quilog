...
export default function PostById({ params: { postId } }) {
  ...
  const handleLikeClick = async () => {
    ...
    if (response.error) {
      ...
      setLiked((currentLikeState) => {
        if (!currentLikeState) {
          data.post.likedBy.push({_id: session.user.id, username: session.user.username, image: session.user.image});
    ...
    if (response.error) {
      console.log(data.error);
          ...
          data.post.comments.unshift({text: comment, user:{
            _id: session.user.id, username: session.user.username, image: session.user.image
          } });
    }
    setComment("");
  }
      ...
      {likedBy.map((ele, id) => {
        return (
          ...
          >
            ...
            <p className="ml-3 font-bold text-md">{ele.username}</p>
          </Link>
        );
      })}
    </div>
        ...
        return (
          <div key={id} className="p-1 bg-zinc-400/10 m-2 rounded">
            ...
            >
              ...
              <p className="text-sm md:text-md">{ele.user.username}</p>
            </Link>
