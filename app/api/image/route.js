...
export async function GET(req) {
  ...
  try {
    ...
    if (Object.keys(query).length == 0 || (!query.postId && !query.userId)) {
        ...
        (
          ...
          >
            <div tw="flex items-center flex-col justify-center w-1/2 h-full">
              ...
              <h1
                tw="text-[90px] font-extrabold p-0 m-0"
                style={{ fontFamily: '"belanosimaRegular"' }}
              >
                QUILOG
              </h1>
              <p tw="text-3xl p-0 m-0" style={{ fontFamily: '"UbuntuLight"' }}>
                Let{`'`}s blog it{" "}
              </p>
              <div
            ...
            <div tw="flex flex-col justify-center items-center w-1/2 h-full p-[48px] my-auto text-black bg-white overflow-hidden">
              <h1
                tw="text-5xl font-extrabold pb-2 border-b-4 border-zinc-400/50 leading-10"
                style={{ fontFamily: '"belanosimaRegular"' }}
              >
                {post.title}
              </h1>
              <p
                tw="m-0 p-0 ml-auto text-xl"
                style={{ fontFamily: '"TypewriterB"' }}
              ...
              <div tw="flex w-full items-center">
                ...
                />
                <h1
                  tw="mr-auto flex text-6xl items-center h-full "
                  style={{ fontFamily: '"belanosimaRegular"' }}
                >
                  {post.user.name}
                </h1>
    ...
    if (query.userId) {
      const user = await User.findOne({ _id: query.userId })
        .select("_id name image likedPosts posts bio")
        .lean(true);
      let name = user.name;
      name = name.split(" ")[0];
      if (name.length > 10) {
        ...
        (
          ...
          >
            ...
            <div tw="flex flex-col justify-center items-center w-1/2 h-full p-[48px] my-auto text-black bg-white overflow-hidden">
              ...
              />
              <h1
                tw="m-0 mx-auto text-6xl border-b-4 border-zinc-400/50"
                style={{ fontFamily: '"belanosimaRegular"' }}
              >
                {user.name}
              </h1>
              {user.bio && (
                <p
                  tw="text-white text-[25px] m-0 mt-2 p-0 text-center  leading-8"
                  style={{ fontFamily: '"UbuntuLight"' }}
                >
                  {user.bio}
                </p>
              ...
              <p
                ...
                style={{ fontFamily: '"TypewriterB"' }}
              >{`${name}'s Created Posts = ${user.posts.length}`}</p>
              <p
                tw="w-full text-[30px] m-2 h-fit"
                style={{ fontFamily: '"TypewriterB"' }}
              >{`${name}'s Liked Posts = ${user.likedPosts.length}`}</p>
            </div>
          </div>
        ),
        // {debug: true},
